...
contract TokenFarm is ChainlinkClient, Ownable {
        ...
        returns (uint256)
    {
        ...
        (uint256 price, uint8 decimals) = getTokenEthPrice(token); //币价
    ...
    function issueTokens() public onlyOwner {
        ...
        for (
            ...
            // 这行是合约调用 transfer，所以是合约给用户发币，而不是合约作者给用户发币
            // 如果合约不想给用户发币，希望合约作者来发，则可以用 transferFrom
            // 合约作者先授权给合约，允许合约用自己的币
            // 合约使用 transferFrom 将合约作者的币转给用户
        }
    }
