...
def deploy_token_farm_and_dapp_token(update_front_end_flag=False):
    account = get_account()
    dapp_token = DappToken.deploy({"from": account})# 部署之后，owner 获得 totalSupply 个 dapp_token
    ...
    ) # owner 留下 KEPT_BALANCE 个 dapp_token，其他的 dapp_token 都转移给应用合约 token_farm
    # owner 给 token_farm 的 dapp_token 有什么用？这个转账并不是 stake。合约为什么要持有这么多 dapp_token ？
    # 其他用户怎么获得 dapp_token？跟合约买吗？怎么买？
    # 合约的作用就是把 dapp_token 转化为 eth，然后 owner 提现 eth 进行赚钱？怎么提现？
    tx.wait(1)
