...
def test_stake_tokens(amount_staked):
    ...
    token_farm, dapp_token = deploy_token_farm_and_dapp_token()
    # Act
    dapp_token.approve(token_farm.address, amount_staked, {"from": account}) # 授权给合约使用 account 的 amount_staked 个代币
    token_farm.stakeTokens(amount_staked, dapp_token.address, {"from": account}) # 合约将 account 的 amount_staked 个代币抵押了
    # Assert
    assert (
...
def test_unstake_tokens(amount_staked):
    ...
    # Act
    token_farm.unstakeTokens(dapp_token.address, {"from": account}) # 撤销所有抵押的币
    # Assert
...
def test_issue_tokens(amount_staked):
    ...
    # Act
    token_farm.issueTokens({"from": account}) # 提现利息
    # Assert
    assert (
